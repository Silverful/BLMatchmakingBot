...
TESSERACT_CMD_DIR = os.getenv("TESSERACT_CMD_DIR", False)
MATCHMAKING_PLAYERBASE_SHEET = os.getenv("MATCHMAKING_PLAYERBASE_SHEET", False)
...
playerbase = MATCHMAKING_PLAYERBASE_SHEET
stats_worksheet = 'Stats'
screens_worksheet = 'Screens'
test_worksheet = 'Test BOT'
...
screenshot_channel_id = 909532047553490984
queue_voice_channel_name = 'Queue'
hinq_guild_id = 545964006851084290

emoji_completed = '\U00002705'
...
mmr_sheets_formula = '=IFERROR((((D%s * -1) + 1 + QUOTIENT(((D%s - 1) * ' + additional_mmr_bank_cell +\
                     ' * 2); 6) + (2 * ' + main_mmr_bank_cell + ' * D%s) - ' + main_mmr_bank_cell + \
                     ') + QUOTIENT((' + additional_mmr_bank_cell + ' * H%s); ' \
                     'SUM(FILTER(H:H; L:L = L%s; D:D = D%s)))) * IF((M%s + 1) = 1; 1; 3);  ' + main_mmr_bank_cell\
                     + ' * IF(D%s = 1; 1; -1) * IF((M%s + 1) = 1; 1; 3))'
#    % (row_number x 9)
