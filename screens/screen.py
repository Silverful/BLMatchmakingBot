...
class Screen:
    ...
    def __init__(self, img):
       ...
       # pytesseract.pytesseract.tesseract_cmd = "/app/.apt/usr/bin/tesseract"
        ...
        self.rounds = None
        self.team1_result = None
        self.team2_result = None

    def find_errors(self):

        players = self.team1_result + self.team2_result
        key = False

        for player in players:

     #       if not str(player['kills']).lstrip('-').isdigit():
      #          key = 'kills'
       #     elif not str(player['assists']).lstrip('-').isdigit():
        #        key = 'assists'
            if not str(player['score']).lstrip('-').isdigit():
                key = 'score'
            elif not player['faction'] in faction_list:
                key = 'faction'

        return key

    def find_score(self):
        upper_row = self.left_pos[1] - self.screenW//17
        if upper_row <= 0:
            upper_row = 0
        scores = self.screen_raw[upper_row:self.left_pos[1] - self.screenW//50,
        ...
        team1_score = data[0]
        team2_score = data[1]
        for scr in 1, 2, 3, 0, 7:
            if str(scr) in str(team1_score) or str(scr) == str(team1_score):
                if scr == 7:
                    self.team1_score = 1
                else:
                    self.team1_score = int(scr)
            if str(scr) in str(team2_score) or str(scr) == str(team2_score):
                if scr == 7:
                    self.team2_score = 1
                else:
                    self.team2_score = int(scr)
        try:
            self.rounds = int(self.team1_score) + int(self.team2_score)
        except:
            raise FacScoreError
    ...
    def split_by_teams(self):
        ...
        pingline = ocvprep.contrast(pingline, contrast=2.5)
    ...
    def get_factions(self):
        upper_row = self.left_pos[1] - (self.stat_len * 7 // 100)
        if upper_row <= 0:
            upper_row = 0

        fac1 = self.screen_raw[upper_row:
                               self.left_pos[1] - (self.stat_len * 36 // 1000),
                               self.left_pos[0]:
                               self.left_pos[0] + self.stat_len * 11 // 100]

        fac2 = self.screen_raw[upper_row:
    ...
    def get_player_data(self, team_no, fac=None):
        ...
        for n in range(6):
            ...
            player_string = team[rowh * n: rowh * (n + 1), :]
            player_name = player_string[:, :player_string.shape[1] * 2 // 5]
            player_kills = player_string[:, player_string.shape[1] * 40 // 100:player_string.shape[1] * 26 // 50]
            player_assists = player_string[:, player_string.shape[1] * 25 // 50:player_string.shape[1] * 32 // 50]
            ...
            player_name = ocvprep.remove_noise(player_name)
            #if is_dead(player_string):
             #    cv.imshow('adf', player_kills)
              #  cv.waitKey(0)

            try:
                player_scores = ocvprep.find_box(player_scores, indent=6)
                player_kills = ocvprep.find_box(player_kills, indent=4)
                player_assists = ocvprep.find_box(player_assists, indent=4)
        ...
        for index, item in enumerate(team1_data):
            ...
            for stat in item[1:4]:
                ...
                if stat:
                    ...
                    string += '-'.ljust(6)
        if team_no == 1:
            self.team1_result = result
        else:
            self.team2_result = result
